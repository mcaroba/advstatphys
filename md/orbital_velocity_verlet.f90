...
! BC
  ...
  open(unit=10, file="orbit_velocity_verlet", status="unknown")
! Run for n time steps
  do step = 0, n
    ...
    do i = 1, 3
      if( step == 0 )then
        call velocity_verlet_pos(pos(i, 1:3), new_vel, fi_sum(i,1:3), m(i), dt, &
                                 new_pos)
        fi_prev(i, 1:3) = fi_sum(i, 1:3)
        pos(i, 1:3) = new_pos(1:3)
      else
        fi_array(1, 1:3) = fi_prev(i, 1:3)
        fi_array(2, 1:3) = fi_sum(i, 1:3)
!       Note that this velocity trails the position by one time step
        call velocity_verlet_vel(vel(i, 1:3), fi_array, m(i), dt, new_vel)
        call velocity_verlet_pos(pos(i, 1:3), new_vel, fi_sum(i,1:3), m(i), dt, &
                                 new_pos)
        fi_prev(i, 1:3) = fi_sum(i, 1:3)
        pos(i, 1:3) = new_pos(1:3)
