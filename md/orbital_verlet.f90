...
! BC
  ...
  open(unit=10, file="orbit_verlet", status="unknown")
! Run for n time steps
  do step = 0, n
    ...
    do i = 1, 3
      if( step == 0 )then
        xi_prev(i, 1:3) = pos(i, 1:3)
        call lazy_man(pos(i, 1:3), vel(i, 1:3), fi_sum(i, 1:3), m(i), dt, &
                          new_pos, new_vel)
      else
        xi_array(1, 1:3) = xi_prev(i, 1:3)
        xi_array(2, 1:3) = pos(i, 1:3)
        call verlet(xi_array, fi_sum(i, 1:3), m(i), dt, new_pos)
      end if
      xi_prev(i, 1:3) = pos(i, 1:3)
