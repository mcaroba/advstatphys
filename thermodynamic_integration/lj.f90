...
! Copyright by Miguel A. Caro (2020)
  ...
  real*8 :: lambda, dl, dHdl, Epot_plus, Epot_minus, Ep_plus, Ep_minus, Ek2
  real*8 :: dHdl_anal

  read(*,*) Nt, Np, L(1), T, mode, lambda
  L(2) = L(1)
  L(3) = L(1)
...
!   Rebuild list every now and then
    ...
    do i = 1, Np
      do k = 1, nn(i)
        ...
        if( j > i )then
          if( mode == "md" )then
!           Decouple one particle only
            if( i == 1 )then
!             Compute potential energy at lambda+dl
              call lj_potential(pos(1:3, i), pos(1:3, j), sigma, sigma, (1.d0-(lambda+dl))*epsilon, &
                                (1.d0-(lambda+dl))*epsilon, Rcut, L, PBC, Epot_plus, fi)
!             Compute potential energy at lambda-dl
              call lj_potential(pos(1:3, i), pos(1:3, j), sigma, sigma, (1.d0-(lambda-dl))*epsilon, &
                                (1.d0-(lambda-dl))*epsilon, Rcut, L, PBC, Epot_minus, fi)
!             Compute potential energy and forces for lambda
              call lj_potential(pos(1:3, i), pos(1:3, j), sigma, sigma, (1.d0-lambda)*epsilon, &
                                (1.d0-lambda)*epsilon, Rcut, L, PBC, Epot, fi)
            else
          ...
          Ep = Ep + Epot
          if( i == 1 )then
            Ep_plus = Ep_plus + Epot_plus
            Ep_minus = Ep_minus + Epot_minus
!           Analytical derivative
            if( lambda /= 1.d0 )then
              dHdl_anal = dHdl_anal - Epot / (1.d0 - lambda)
            end if
          end if
        end if
      end do
...
!       Interpolate the velocity to estimate the temperature
        ...
        if( i == 1 )then
          Ek2 = Ek2 + 0.5d0 * M(i) * dot_product(vel(1:3, i), vel(1:3, i))
        else
...
!     We use one thermostat for each subsystem. We also distribute the missing DoFs (due
!     to CM velocity removal) proportionaly to each subsystem
      call berendsen_thermostat(vel(1:3,2:Np), T, 2.d0/3.d0/(dfloat(Np-1)**2/dfloat(Np))/kB*Ek, &
                                tau, dt)
      call berendsen_thermostat(vel(1:3,1:1), T, 2.d0/3.d0/(dfloat(Np-1)/dfloat(Np))/kB*Ek2, &
                                tau, dt)
...
!   Write time, potential energy, kinetic energy, instantaneous temperature and dH/dl
    if( int(step/write_log)*write_log == step )then
      write(10,*) dfloat(step)*dt, Ep, Ek, 2.d0/3.d0/dfloat(Np-1)/kB*Ek, dHdl, dHdl_anal
    end if
  end do
