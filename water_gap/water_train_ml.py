...
CTS = np.zeros([Ntrain+Ntest+Nvalidate, 3*Nsparse])
for t in range(0, Ntrain+Ntest+Nvalidate):
...
# This is just the part of the matrix we use for training
CTS_train = CTS[0:Ntrain,:]

# This is the regularizing matrix
LTT_inv = np.zeros([Ntrain,Ntrain])
...
alphas = np.dot(C_temp, energies[0:Ntrain])

# Now we make the predictions
pred = np.dot(CTS[0:Ntrain+Ntest+Nvalidate], alphas)
# For the training set
f = open("training_set.dat", "w+")
rmse_train = 0.
for i in range(0, Ntrain):
    rmse_train += (energies[i]-pred[i])**2
    print("%f %f" % (energies[i], pred[i]), file=f)

f.close()
rmse_train = np.sqrt( rmse_train/float(Ntrain) )

# For the test set
f = open("test_set.dat", "w+")
rmse_test = 0.
for i in range(Ntrain, Ntrain+Ntest):
    rmse_test += (energies[i]-pred[i])**2
    print("%f %f" % (energies[i], pred[i]), file=f)

f.close()
rmse_test = np.sqrt( rmse_test/float(Ntest) )

# For the validation set
f = open("validation_set.dat", "w+")
rmse_validate = 0.
for i in range(Ntrain+Ntest,Ntrain+Ntest+Nvalidate):
    rmse_validate += (energies[i]-pred[i])**2
    print("%f %f" % (energies[i], pred[i]), file=f)

f.close()
rmse_validate = np.sqrt( rmse_validate/float(Nvalidate) )

# Print hyperparameters and errors
print("%f %f %f %f %f" % (sigma2b, sigma, rmse_train, rmse_test, rmse_validate))
